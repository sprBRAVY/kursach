...
namespace PrintingOrderManager.Application.Services
{
    public class PaymentService : IPaymentService
    {
        ...
        public IQueryable<PaymentDto> GetPaymentsQueryable()
        {
            // PaymentDto не содержит связанных сущностей → Include не нужен
            return _paymentRepository.GetQueryable()
        ...
        {
            var payments = await _paymentRepository.GetAllWithOrderAsync();
            var filtered = payments.Where(p => p.Amount >= minAmount && p.Amount <= maxAmount);
